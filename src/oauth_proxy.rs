...
struct Inner {
    ...
    /// Forced RFC 8707 resource sent to Logto: this MCP server's origin.
    resource_url: String,
...
///
/// Clients may send `{origin}` or `{origin}/mcp`; Logto's API resource is the
/// origin, so normalize the authorization and token requests to that value.
fn apply_resource(pairs: &mut Vec<(String, String)>, resource_url: &str) {
    if !crate::config::is_absolute_http_uri(resource_url) {
        return;
    }
    let mut saw = false;
    for (k, v) in pairs.iter_mut() {
        if k == "resource" {
            resource_url.clone_into(v);
            saw = true;
        }
    }
    if !saw {
        pairs.push(("resource".to_owned(), resource_url.to_owned()));
    }
}
...
pub async fn authorize(State(st): State<OAuthProxyState>, RawQuery(q): RawQuery) -> Response {
    ...
    apply_resource(&mut pairs, &st.inner.resource_url);
...
mod tests {
    ...
    fn callback_url_derived_from_resource() {
        ...
        assert_eq!(
            ...
            "https://hevy-mcp.oddie.app/oauth/callback"
        );
    ...
    async fn authorize_forwards_only_allowed_redirect_uri_to_pending_state() {
        ...
        let response = authorize(
            State(st.clone()),
            RawQuery(Some(
                "client_id=abc&redirect_uri=https%3A%2F%2Fclaude.ai%2Fapi%2Fmcp%2Fauth_callback&state=client-state&resource=https%3A%2F%2Fhevy-mcp.oddie.app%2Fmcp"
                    .to_owned(),
            )),
        )
        .await;
        ...
        assert_eq!(
            ...
            Some("https://hevy-mcp.oddie.app/oauth/callback")
        );
        let resource = params.get("resource").map(String::as_str);
        assert_eq!(resource, Some("https://hevy-mcp.oddie.app"));
    ...
    #[test]
    fn apply_resource_rewrites_mcp_path_to_origin() {
        const URI: &str = "https://hevy-mcp.oddie.app";
        let mut pairs = vec![(
            "resource".to_owned(),
            "https://hevy-mcp.oddie.app/mcp".to_owned(),
        )];
        apply_resource(&mut pairs, URI);
        assert_eq!(pairs, vec![("resource".to_owned(), URI.to_owned())]);
        assert!(Url::parse(&pairs[0].1).unwrap().has_host());
        let mut pairs = vec![(
            "resource".to_owned(),
            "https://hevy-mcp.oddie.app/".to_owned(),
        )];
        apply_resource(&mut pairs, URI);
        assert_eq!(pairs, vec![("resource".to_owned(), URI.to_owned())]);
    }
    ...
    #[test]
    fn apply_resource_inserts_origin_when_missing() {
        const URI: &str = "https://hevy-mcp.oddie.app";
