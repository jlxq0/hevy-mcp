...
//! public-SPA client (`HEVY_MCP_DCR_CLIENT_ID`, redirect URIs already
...
/// `HEVY_MCP_RESOURCE_URL` to include `/mcp`.
...
mod tests {
    ...
    fn protected_resource_points_at_self() {
        ...
        assert_eq!(m.resource, "https://hevy-mcp.oddie.app/mcp");
        // We front the IdP: authorization_servers must be our own origin so
        // claude.ai fetches our DCR-enabled metadata.
        assert_eq!(m.authorization_servers, vec!["https://hevy-mcp.oddie.app"]);
    ...
    fn as_metadata_is_same_origin_with_jwks_on_logto() {
        ...
        assert_eq!(m.issuer, "https://hevy-mcp.oddie.app");
        // authorize/token are on OUR origin (proxied); jwks stays on Logto.
        assert_eq!(
            m.authorization_endpoint,
            "https://hevy-mcp.oddie.app/authorize"
        );
        assert_eq!(m.token_endpoint, "https://hevy-mcp.oddie.app/token");
    ...
    fn as_metadata_advertises_registration_when_configured() {
        ...
        assert_eq!(
            ...
            Some("https://hevy-mcp.oddie.app/register")
        );
    }
    ...
    fn www_authenticate_includes_resource_metadata_url() {
        let h = www_authenticate_header("https://hevy-mcp.oddie.app");
        assert!(h.starts_with("Bearer "));
        assert!(h.contains(
            r#"resource_metadata="https://hevy-mcp.oddie.app/.well-known/oauth-protected-resource/mcp""#
        ));
    }
}
