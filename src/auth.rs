...
/// by `bearer_auth`. It authenticates the MCP caller to this service; Hevy
/// requests use the separately configured Hevy API key.
#[derive(Clone)]
...
mod tests {
    ...
    async fn unauthorized_has_www_authenticate() {
        let r = unauthorized("https://hevy-mcp.oddie.app");
